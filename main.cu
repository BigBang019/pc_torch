...
std::vector<at::Tensor> read() {
    ...
    defer {pc_file.close(); face_file.close(); };
    ...
    return std::vector<at::Tensor>{pc, faces};
}
...
void test_graph_neigbor_query() {
    ...
    DEBUG(pc[batch_idx]);
    DEBUG(faces[batch_idx]);
    auto idx = graph_neighbor_query(pc, faces, nsample);
    sleep(1);
    DEBUG(idx[batch_idx]);
